...
<div id="body">
	<div id="serviceMenu" align="center">
		<div class="selected qna"><a href="/qnabd/list">자주 묻는 질문</a>
		</div><div class="inqubd"><a href="/inqubd/list">문의사항</a>
	</div></div>
		<div id="qnaboard">
			<c:forEach items="${list}" var="list">
				<details class="qnabdList" title="${list.qnabd_no}">
					<summary class="q" title="${list.qnabd_no}"><strong>Q? </strong><span>${list.qnabd_q}</span>
					<sec:authorize access="hasAuthority('ROLE_ADMIN')"><button class="qnaupdate" title="${list.qnabd_no}">수정</button> 
					<button class="qnadelete" title="${list.qnabd_no}">삭제</button></sec:authorize></summary>
					<div class="a"><strong>  A : </strong><span>${list.qnabd_a}</span></div>
				</details>
			</c:forEach>
		</div>
